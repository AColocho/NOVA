...
Behavior:
  - By default, starts frontend, backend, Postgres, MinIO, and the MinIO bucket init job.
...
if [[ "${#INFRA_SERVICES[@]}" -gt 0 ]]; then
  docker compose up -d "${COMPOSE_ARGS[@]}" "${INFRA_SERVICES[@]}"
fi

docker compose up -d "${COMPOSE_ARGS[@]}" "${APP_SERVICES[@]}"

if [[ "$RUN_DB_SETUP" -eq 1 ]]; then
  docker compose exec -T backend uv run python -m backend.db_management.setup
fi

echo "NOVA containers are up."
